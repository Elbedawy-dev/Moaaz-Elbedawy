...
const timeline = [
  ...
  },
  {
    ...
    description:
      '2-year Higher Diploma program building a strong foundation in computer science and software development.',
  },
  {
    icon: Briefcase,
  ...
  },
  {
    ...
    description:
      'Building real-world projects like a POS System and Notes App, applying React, Node.js, Express, and MongoDB end to end.',
  },
]
...
export default function About() {
  ...
  return (
    <>
      ...
      {/* PERSONAL INFO + STATS */}
      <section className="mx-auto max-w-content px-5 pb-16 lg:grid lg:grid-cols-2 lg:gap-8 lg:pb-20">
        <motion.div
          initial={reduceMotion ? false : 'hidden'}
          whileInView="show"
          viewport={{ once: true, margin: '-80px' }}
        ...
        >
          ...
          {/* TODO: CV_URL currently points at src/pdf/CV.pdf (same file used on the Home hero) — swap in an updated CV when available */}
          
          <a href={CV_URL}
            download
            className="mt-6 inline-flex items-center gap-2 rounded-control bg-accent px-5 py-2.5 text-sm font-semibold text-text-primary shadow-glow transition-all hover:bg-accent-hover hover:shadow-glow-strong"
          >
            <Download size={16} />
            Download CV
          </a>
        ...
        >
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="rounded-card border border-border bg-surface-1 p-5 transition-all duration-300 
              hover:-translate-y-0.5 hover:border-border-active hover:shadow-card-hover"
            >
